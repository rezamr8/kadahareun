angular.module("aneka_resep_kue.controllers", [])
...
.controller("indexCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture){
	...
	popover_template += "			<a  class=\"item dark-ink\" ng-click=\"openURL('market://details?id=com.e74Studio.kumpulan.anekaresepkue')\">";
	popover_template += "			Rate Us";
	popover_template += "			</a>";
	popover_template += "		</ion-list>";
	popover_template += "	</ion-content>";
...
// TODO: resep_kueCtrl --|-- 
.controller("resep_kueCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture){
	...
	$rootScope.last_edit = "table (resep)" ;
	$scope.$on("$ionicView.afterEnter", function (){
		var page_id = $state.current.name ;
		$rootScope.page_id = page_id.replace(".","-") ;
	});
	...
	// TODO: resep_kueCtrl --|-- $scope.scrollTop
	$rootScope.scrollTop = function(){
		$timeout(function(){
			$ionicScrollDelegate.$getByHandle("top").scrollTop();
		},100);
	};
	// TODO: resep_kueCtrl --|-- $scope.openURL
	// open external browser 
	$scope.openURL = function($url){
		window.open($url,"_system","location=yes");
	};
	// TODO: resep_kueCtrl --|-- $scope.openAppBrowser
	// open AppBrowser
	$scope.openAppBrowser = function($url){
		var appBrowser = window.open($url,"_blank","hardwareback=Done");
		appBrowser.addEventListener("loadstart",function(){
	...
	// TODO: resep_kueCtrl --|-- $scope.openWebView
	// open WebView
	$scope.openWebView = function($url){
		var appWebview = window.open($url,"_blank","location=no");
		appWebview.addEventListener("loadstart",function(){
	...
	// TODO: resep_kueCtrl --|-- $scope.toggleGroup
	$scope.toggleGroup = function(group) {
		if ($scope.isGroupShown(group)) {
			$scope.shownGroup = null;
		} else {
	...
	// TODO: resep_kueCtrl --|-- $scope.redirect
	// redirect
	$scope.redirect = function($url){
		$window.location.href = $url;
	};
	...
	// TODO: resep_kueCtrl --|-- $scope.showAuthentication
		...
		var authPopup = $ionicPopup.show({
			...
			buttons: [
				{text:"Cancel",onTap: function(e){
					$state.go("aneka_resep_kue.resep_kue");
				}},
			],
		}).then(function(form){
		},function(err){
	...
	// TODO: resep_kueCtrl --|-- $scope.splitArray
	...
	// TODO: resep_kueCtrl --|-- $scope.fetchURL
	$scope.fetchURL = "data/tables/resep.json";
	// TODO: resep_kueCtrl --|-- $scope.fetchURLp
	$scope.fetchURLp = "data/tables/resep.json?callback=JSON_CALLBACK";
	// TODO: resep_kueCtrl --|-- $scope.hashURL
	...
	localforage.getItem("data_reseps_" + $scope.hashURL, function(err, get_reseps){
		if(get_reseps === null){
			data_reseps =[];
		}else{
			data_reseps = JSON.parse(get_reseps);
			$scope.data_reseps =JSON.parse( get_reseps);
			$scope.reseps = [];
			for(lastPush = 0; lastPush < 50; lastPush++) {
				if (angular.isObject(data_reseps[lastPush])){
					$scope.reseps.push(data_reseps[lastPush]);
				};
			}
			$timeout(function() {
				$ionicLoading.hide();
	...
	if(data_reseps.length === 0 ){
		$timeout(function() {
			...
			$http.get(url_request,http_header).then(function(response) {
				data_reseps = response.data;
				console.log("%cSuccessfully","color:blue;font-size:18px");
				console.dir(data_reseps);
				$scope.data_reseps = response.data;
				// TODO: resep_kueCtrl --|---------- set:localforage
				localforage.setItem("data_reseps_" + $scope.hashURL, JSON.stringify(data_reseps));
				$scope.reseps = [];
				for(lastPush = 0; lastPush < 50; lastPush++) {
					if (angular.isObject(data_reseps[lastPush])){
						$scope.reseps.push(data_reseps[lastPush]);
				...
				$timeout(function() {
					...
					if(response.status ===401){
						// TODO: resep_kueCtrl --|------------ error:Unauthorized
						$scope.showAuthentication();
					}else{
						// TODO: resep_kueCtrl --|------------ error:Message
	...
	// TODO: resep_kueCtrl --|-- $scope.doRefresh
	...
	// TODO: resep_kueCtrl --|-- controller_by_user
	// controller by user 
	function controller_by_user(){
		try {
			
//debug: all data
//console.log(data_reseps);
$ionicConfig.backButton.text("");
			
		} catch(e){
			console.log("%cerror: %cPage: `resep_kue` and field: `Custom Controller`","color:blue;font-size:18px","color:red;font-size:18px");
			console.dir(e);
		}
	}
	$scope.rating = {};
...
.controller("resep_singlesCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture){
	...
	// TODO: resep_singlesCtrl --|-- $scope.scrollTop
	$rootScope.scrollTop = function(){
		$timeout(function(){
			$ionicScrollDelegate.$getByHandle("top").scrollTop();
		},100);
	};
	// TODO: resep_singlesCtrl --|-- $scope.openURL
	// open external browser 
	$scope.openURL = function($url){
		window.open($url,"_system","location=yes");
	};
	// TODO: resep_singlesCtrl --|-- $scope.openAppBrowser
	// open AppBrowser
	$scope.openAppBrowser = function($url){
		var appBrowser = window.open($url,"_blank","hardwareback=Done");
		appBrowser.addEventListener("loadstart",function(){
	...
	// TODO: resep_singlesCtrl --|-- $scope.openWebView
	// open WebView
	$scope.openWebView = function($url){
		var appWebview = window.open($url,"_blank","location=no");
		appWebview.addEventListener("loadstart",function(){
	...
	// TODO: resep_singlesCtrl --|-- $scope.toggleGroup
	$scope.toggleGroup = function(group) {
		if ($scope.isGroupShown(group)) {
			$scope.shownGroup = null;
		} else {
	...
	// TODO: resep_singlesCtrl --|-- $scope.redirect
	// redirect
	$scope.redirect = function($url){
		$window.location.href = $url;
	};
	...
	// TODO: resep_singlesCtrl --|-- $scope.showAuthentication
	$scope.showAuthentication  = function(){
		var authPopup = $ionicPopup.show({
			template: ' This page required login',
			title: "Authorization",
	...
	// TODO: resep_singlesCtrl --|-- $scope.fetchURL
	$scope.fetchURL = "data/tables/resep.json";
	// TODO: resep_singlesCtrl --|-- $scope.fetchURLp
	$scope.fetchURLp = "data/tables/resep.json?callback=JSON_CALLBACK";
	// TODO: resep_singlesCtrl --|-- $scope.hashURL
	$scope.hashURL = md5.createHash($scope.fetchURL);
	
	var current_item = [];
	localforage.getItem("data_reseps_" + $scope.hashURL, function(err, get_datas){
		if(get_datas === null){
			current_item = [];
		}else{
			if(get_datas !== null){
	...
	if( current_item.length === 0 ){
		...
		// TODO: resep_singlesCtrl --|-- $http.get
		...
		// TODO: resep_singlesCtrl --|-- $scope.doRefresh
	$scope.doRefresh = function(){
		// Retrieving data
		var itemID = $stateParams.id;
		var current_item = [];
		...
		$http.get($scope.fetchURL,http_header).then(function(response) {
			// Get data single
			var datas = response.data;
			// TODO: resep_singlesCtrl --|---------- set:localforage
			localforage.setItem("data_reseps_"+ $scope.hashURL,JSON.stringify(datas));
			for (var i = 0; i < datas.length; i++) {
				if((datas[i].id ===  parseInt(itemID)) || (datas[i].id === itemID.toString())) {
					current_item = datas[i] ;
				}
			...
			// Error message
		// TODO: resep_singlesCtrl --|---------- $http.jsonp
	...
	// TODO: resep_singlesCtrl --|-- controller_by_user
	// controller by user 
	function controller_by_user(){
		try {
...
$ionicConfig.backButton.text("");			
			...
			console.log("%cerror: %cPage: `resep_singles` and field: `Custom Controller`","color:blue;font-size:18px","color:red;font-size:18px");
			console.dir(e);
		}
	}
	$scope.rating = {};
